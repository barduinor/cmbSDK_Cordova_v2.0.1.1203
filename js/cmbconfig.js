...
//check if scanner is available, if it is connect automatically to it
     this.scanner.setAvailabilityCallback(function(readerAvailability){
           if (readerAvailability == this.scanner.CONSTANTS.AVAILABILITY_AVAILABLE) {
               me.scanner.connect();
           }else{
...
//it will return TRUE in the result if the scanning process is STARTED and false if it's NOT STARTED
...
//example:
               document.getElementById('content').insertAdjacentHTML('beforeend','<div class="result"><span class="symbol">'+result.symbology+'</span> : '+result.readString+'</div>');
