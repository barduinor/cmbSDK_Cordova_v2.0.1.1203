    ...
    private void loadScanner(final CallbackContext callbackContext) {
        ...
        cordova.getActivity().runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                if (listeningForUSB) {
                    cordova.getActivity().unregisterReceiver(mxConnectedReceiver);
                ...
                if (deviceTypeFromInt(param_deviceType) == DeviceType.MOBILE_DEVICE) {
                    ...
                    if (!listeningForUSB) {
                        cordova.getActivity().registerReceiver(mxConnectedReceiver, new IntentFilter(MXHelper.ACTION_MX_ATTACHED));
...
//                });
    ...
    private void updateScannerViewPosition() {
        if (scannerView != null) {
            FrameLayout.LayoutParams scannerViewParams = new FrameLayout.LayoutParams(Math.round(param_sizeWidth), Math.round(param_sizeHeight));
            ...
            try {
                ...
                if (viewObject instanceof View) {
                    ...
                    if (parentView instanceof ViewGroup) {
                        mainViewGroup = (ViewGroup) parentView;
...
//        LOG.d("Lazyvlad",readResults.getResultAt(0).getReadString());
    ...
    private BroadcastReceiver mxConnectedReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            if (MXHelper.ACTION_MX_ATTACHED.equals(intent.getAction())) {
                if (availabilityDidChangeOfReaderCallbackID != null) {
                    PluginResult pr = new PluginResult(PluginResult.Status.OK, ReaderDevice.Availability.AVAILABLE.ordinal());
                    pr.setKeepCallback(true);
                    availabilityDidChangeOfReaderCallbackID.sendPluginResult(pr);
                }
            } else {
                ...
                if (availabilityDidChangeOfReaderCallbackID != null) {
                    PluginResult pr = new PluginResult(PluginResult.Status.OK, ReaderDevice.Availability.UNAVAILABLE.ordinal());
                    pr.setKeepCallback(true);
                    availabilityDidChangeOfReaderCallbackID.sendPluginResult(pr);
                    disconnect(null);
                }
